...
template <typename T>
queue<T>::node::node(T value) : value(value), next(nullptr) {}
...
template <typename T>
queue<T>::queue(const int max_capacity) : head_(nullptr), max_capacity_(max_capacity) {}

template <typename T>
queue<T>::queue(T values[]) : head_(nullptr), max_capacity_(sizeof(values) / sizeof(T))
{
	push(values);
}

template <typename T>
queue<T>::queue(const queue& other) : head_(nullptr)
{
